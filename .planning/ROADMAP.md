...
| 8 | Runner ↔ evolution wiring + runner-debt cleanup + phase-7 real-LLM hardening | Pending | 2026-05-27 — `08-CHARTER.md` covers three groups: A-E phase-7 real-LLM hardening (timeout 60→180s, request-level transient retry, CR-05 audit, runner `--env-file`, `failure_class` column), F runner ↔ evolution wiring with seeded test delta (primary deliverable), G 7 WR/IN runner-debt items deferred from phase 7. Single runner-touch sweep so phase 7 can re-launch Stage 1+2+3 cleanly. |
