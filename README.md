...
```mermaid
flowchart TB
    CSV["CSV<br/>request / list_group"] --> N1["① 用户挖掘"]
    N1 --> N2["② 文案生成"]
    N2 --> N3["③ Rubric 判分"]
    N3 --> OUT["离线落表<br/>request_id,user_id,item_id,copy"]
    N3 --> TRAJ["轨迹沉淀<br/>每条请求 = 一条轨迹"]
    TRAJ --> DIS["蒸馏<br/>跨轨迹找系统性弱点"]
    DIS --> DELTA["Delta 补丁<br/>可执行 / 可审计"]
    DELTA --> TRIAL["小流量试验<br/>≤ 2% 流量"]
    TRIAL --> POST["贝叶斯后验"]
    POST -- "置信度 ≥ 0.975" --> PROM["合入线上 skill<br/>带归档 / 回滚"]
    PROM -. "改进生产" .-> N2
    POST -. "证据不足，继续试" .-> TRIAL
    ...
    class CSV,N1,N2,N3,OUT prod;
    class DIS,DELTA,TRIAL,POST evo;
    class TRAJ bridge;
    class PROM pass;
```
...
```mermaid
flowchart TB
    A["CSV / request_id 列表"] --> B["请求预处理<br/>同 request 打包合规商品"]
    B --> C["① 用户个性化挖掘<br/>personalized-user-mining"]
    C --> D["② 文案生成<br/>personalized-copy-generation"]
    D --> E["③ Rubric 打分<br/>personalized-copy-rubric-judge"]
    E --> F["确定性校验<br/>字段 / 结构 / 业务约束"]
    F --> G["离线落表<br/>offline_copy_table.csv / jsonl"]
    F --> H["轨迹与证据<br/>index / summary / evidence"]
    H --> I{"累积到进化阈值?"}
    I -- "否" --> J["继续生产"]
    I -- "是" --> K["蒸馏生成 Delta<br/>JSON edit"]
    K --> L["patchability 校验<br/>临时 skill workspace"]
    L --> M["小流量 trial<br/>对照正式版本"]
    M --> N["后验更新<br/>portfolio / journal"]
    N --> O{"证据充分?"}
    O -- "否" --> J
    O -- "是" --> P["promotion<br/>写入正式 skill"]
