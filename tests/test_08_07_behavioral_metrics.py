...
from seers_harness.validation.machine_judges import (
    ...
    compute_user_factor_count_p50,
    compute_user_factor_diversity,
    compute_reflection_trigger_rate,
)


def test_machine_judges_user_factor_count_p50() -> None:
    artifacts = [{"user_factors": [{}] * n} for n in [1, 2, 3, 4, 5, 6]]
    assert compute_user_factor_count_p50(artifacts) == 3.5
    assert compute_user_factor_count_p50(
        [{"user_factors": [{}] * n} for n in [2, 2, 3, 3, 3, 3]]
    ) == 3.0
...
def test_machine_judges_factor_diversity_jaccard() -> None:
    diverse = [
        {
            "user_factors": [
                {"signal_basis": "甲", "need_or_pain": "乙"},
                {"signal_basis": "丙", "need_or_pain": "丁"},
            ]
        }
    ]
    identical = [
    ...
    assert compute_user_factor_diversity(diverse) == 1.0
    assert compute_user_factor_diversity(identical) == 0.0
    assert compute_user_factor_diversity([]) == 0.0
    assert compute_user_factor_diversity([{"user_factors": [{}]}]) == 0.0
...
def test_build_behavioral_report_happy_path(tmp_path) -> None:
    ...
    _write_request(
        ...
        "req-1",
        user_factors=[
            {"user_factor_id": "uf1", "signal_basis": "甲", "need_or_pain": "乙"},
            {"user_factor_id": "uf2", "signal_basis": "丙", "need_or_pain": "丁"},
        ],
        candidate_counts=2,
    ...
    _write_request(
        ...
        "req-2",
        user_factors=[
            {"user_factor_id": "uf3", "signal_basis": "戊", "need_or_pain": "己"},
            {"user_factor_id": "uf4", "signal_basis": "庚", "need_or_pain": "辛"},
            {"user_factor_id": "uf5", "signal_basis": "壬", "need_or_pain": "癸"},
            {"user_factor_id": "uf6", "signal_basis": "子", "need_or_pain": "丑"},
        ],
        candidate_counts=3,
        portfolio=[
            {
    ...
    assert set(report) == {
        "user_factor_count_p50",
        "user_factor_diversity_score",
        "copy_candidate_count_p50",
        "reflection_triggered_when_underspec_rate",
        "delta_diversity_score",
        "trial_belief_update_count",
    }
    assert report["user_factor_count_p50"] == 3.0
    assert report["user_factor_diversity_score"] == 1.0
...
def test_build_behavioral_report_handles_missing_evidence(tmp_path) -> None:
    ...
    assert build_behavioral_report(stage_dir) == {
        "user_factor_count_p50": 0.0,
        "user_factor_diversity_score": 0.0,
...
def test_batch_summary_includes_behavioral_metrics(tmp_path) -> None:
    ...
    _write_request(
        stage_dir,
        "req-1",
        user_factors=[{"user_factor_id": "uf1", "signal_basis": "甲", "need_or_pain": "乙"}],
        candidate_counts=2,
    )
...
def _write_request(
    ...
    *,
    ...
    user_dir = request_dir / "evidence/personalized_user_mining"
    user_dir.mkdir(parents=True, exist_ok=True)
    (user_dir / "artifact.json").write_text(
        json.dumps({"user_factors": user_factors}),
        encoding="utf-8",
    )
    (user_dir / "tool_calls.jsonl").write_text(
