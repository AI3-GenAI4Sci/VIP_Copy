...
def test_factor_text_specs_discourage_ascii_quote_marks() -> None:
    record_props = RECORD_FACTOR_SPEC["function"]["parameters"]["properties"]
    submit_props = SUBMIT_FACTORS_FINAL_SPEC["function"]["parameters"]["properties"][
        "factors"
    ]["items"]["properties"]
    ...
    for props, names in (
        (
            record_props,
            ("user_side_signal", "bridge_to_product", "transferable_disposition"),
        ),
        (
            submit_props,
            ("user_side_signal", "bridge", "transferable_disposition"),
        ),
    ):
        for name in names:
            description = props[name].get("description", "")
            assert "ASCII double quote" in description
            assert "Chinese quote marks" in description
